        ...
        <form onSubmit={submitOtp} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="flex justify-between gap-2">
              {digits.map((d, i) => (
                <input key={i} ref={(el) => { inputs.current[i] = el; }} value={d} onChange={(e) => onChange(i, e.target.value)} onKeyDown={(e) => onKeyDown(i, e)} inputMode="numeric" maxLength={1} className="h-14 w-12 rounded-2xl border border-zinc-200 bg-white text-center text-xl font-semibold text-zinc-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-100 sm:w-14" />
              ))}
            </div>
            {error && <p className="mt-2 text-xs text-rose-500">{error}</p>}
          </div>
