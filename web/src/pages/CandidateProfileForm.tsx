...
export function CandidateProfileForm() {
  ...
  const [preferredPlace, setPreferredPlace] = useState<PlaceRef>({
  ...
  /**
   * Display label for the preferred work location, auto-derived from the
   * structured picker (`preferredPlace`). Falls back to the current location's
  ...
  const preferredLocation = (() => {
    if (preferredPlace.talukId) return publicLabel(preferredPlace);
  ...
  const handleNext = () => {
    if (step === 0) {
      ...
      patch(user.id, {
        ...
        preferredDistrictId: preferredPlace.districtId,
        preferredTalukId: preferredPlace.talukId,
        preferredLat: preferredPlace.lat,
        preferredLng: preferredPlace.lng,
        preferredPincode: preferredPlace.pincode,
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-5 dark:border-zinc-800 dark:bg-zinc-950/50">
                <h3 className="mb-1 text-sm font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">
                  Preferred work location <span className="text-xs font-normal text-zinc-500">(optional)</span>
                </h3>
                <p className="mb-3 text-[11px] text-zinc-600 dark:text-zinc-400">
                  Different from where you live? We'll match jobs near both anchors.
                </p>
                <LocationPicker value={preferredPlace} onChange={setPreferredPlace} allowStreet={false} />
              </div>
            </div>
          </StepShell>
        );
