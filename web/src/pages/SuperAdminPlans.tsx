    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      <header className="border-b border-zinc-200 bg-white px-5 py-3 dark:border-zinc-800 dark:bg-zinc-900">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <button onClick={async () => { await logout(); navigate("/"); }} className="inline-flex items-center gap-1.5 rounded-full border border-zinc-200 px-3 py-1.5 text-xs font-semibold text-zinc-700 transition hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800">
      ...
      <main className="mx-auto max-w-7xl px-5 py-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight md:text-4xl">Subscription plans</h1>
        ...
        {/* Create form */}
        <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8 rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <form onSubmit={onCreate} className="grid gap-3 sm:grid-cols-6">
            ...
            <Field className="sm:col-span-2" label="GST applies (18%)">
              <label className="inline-flex h-[44px] cursor-pointer items-center gap-2 rounded-2xl border border-zinc-200 bg-white px-3 dark:border-zinc-800 dark:bg-zinc-950">
                <input type="checkbox" checked={draft.gstApplies} onChange={(e) => setDraft((d) => ({ ...d, gstApplies: e.target.checked }))} className="h-4 w-4 rounded accent-amber-500" />
                <span className="text-sm">{draft.gstApplies ? "Yes — 18% GST on top" : "No"}</span>
              </label>
            </Field>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-6">
            {(Object.keys(grouped) as ApiPlanAudience[]).map((aud) => (
              <section key={aud} className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div className="mb-4 flex items-center gap-2">
                ...
                {grouped[aud].length === 0 ? (
                  ...
                  <ul className="flex flex-col gap-3">
                    {grouped[aud].map((p) => {
                      ...
                      return (
                        <li key={p.id} className={["rounded-2xl border p-4", p.active ? "border-zinc-200 dark:border-zinc-800" : "border-dashed border-zinc-300 bg-zinc-50/40 dark:border-zinc-700 dark:bg-zinc-950/30"].join(" ")}>
                          {e ? (
                            /* ----- Edit mode ----- */
                            <div className="grid gap-3 sm:grid-cols-6">
                              ...
                              <Field label="GST">
                                <label className="inline-flex h-[44px] cursor-pointer items-center gap-2 rounded-2xl border border-zinc-200 bg-white px-3 dark:border-zinc-800 dark:bg-zinc-950">
                                  <input type="checkbox" checked={e.gstApplies} onChange={(ev) => setEditing((cur) => ({ ...cur, [p.id]: { ...e, gstApplies: ev.target.checked } }))} className="h-4 w-4 rounded accent-amber-500" />
                                  <span className="text-xs">{e.gstApplies ? "Yes" : "No"}</span>
                                </label>
                              </Field>
                              ...
                              <div className="sm:col-span-6 flex items-center justify-end gap-2">
                                <button onClick={() => cancelEdit(p.id)} disabled={isSaving} className="rounded-2xl border border-zinc-200 px-3 py-2 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 disabled:opacity-60 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800">Cancel</button>
                            ...
                            <div className="flex flex-wrap items-center gap-4">
                              ...
                              <div className="flex items-center gap-1">
                                <button onClick={() => toggleActive(p)} disabled={isSaving} title={p.active ? "Hide from subscribe pages" : "Show on subscribe pages"} className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-zinc-200 text-zinc-700 hover:bg-zinc-100 disabled:opacity-60 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800">
                                  {p.active ? <ToggleRight size={14} className="text-emerald-600 dark:text-emerald-400" /> : <ToggleLeft size={14} />}
                                </button>
                                <button onClick={() => startEdit(p)} disabled={isSaving} className="rounded-full border border-zinc-200 px-3 py-1 text-[11px] font-semibold text-zinc-700 hover:bg-zinc-100 disabled:opacity-60 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800">Edit</button>
      ...
      <AnimatePresence>
        {confirmingDelete && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setConfirmingDelete(null)}>
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-3xl border border-zinc-200 bg-white p-6 shadow-2xl dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <div className="mt-5 flex justify-end gap-2">
                <button onClick={() => setConfirmingDelete(null)} className="rounded-2xl border border-zinc-200 px-4 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800">Cancel</button>
...
function inputClass(extra = "") {
  return ["w-full rounded-2xl border border-zinc-200 bg-white px-3 py-2.5 text-sm focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20 dark:border-zinc-800 dark:bg-zinc-950", extra].join(" ");
}
