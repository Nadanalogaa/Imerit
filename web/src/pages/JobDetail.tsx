...
export function JobDetail() {
 ...
 // / `hasApplied` functions. Function refs are stable so zustand never
 ...
 isSaved
 ? "border-rose-400 bg-rose-500 text-white shadow-sm shadow-rose-500/30 hover:border-rose-500 hover:bg-rose-600"
 : "border-zinc-200 text-zinc-700 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-200 dark:hover:bg-zinc-800",
 ].join(" ")}
 >
 {isSaved ? (
 <>
 <Bookmark size={14} fill="currentColor" strokeWidth={2.5} className="group-hover:hidden" />
 ...
 className="rounded-3xl bg-white p-6 shadow-[0_4px_16px_rgba(15,23,42,0.06)] dark:shadow-[0_4px_16px_rgba(0,0,0,0.35)] dark:bg-zinc-900 md:p-6 md:p-8"
 >
 ...
 {job.employerName && (
 <p className="mt-1 text-base font-semibold text-sky-700 dark:text-sky-300">{job.employerName}</p>
 )}
 </div>
 </div>
 ...
 >
 <div className="flex flex-col items-center gap-3 rounded-3xl bg-white/90 p-5 shadow-2xl shadow-zinc-300/40 backdrop-blur-xl dark:bg-zinc-900/90 dark:shadow-black/40 sm:flex-row sm:justify-between">
 <div>
 {hasApplied ? (
 <>
...
Subscribe Now.  — Unlock with ₹333/45 days/Unlimited Jobs
 ...
 className="w-full max-w-md rounded-3xl bg-white p-6 md:p-8 text-center shadow-2xl dark:bg-zinc-900"
 >
 <motion.div
 initial={{ scale: 0 }}
 animate={{ scale: 1 }}
 ...
 <button
 onClick={() => setShowApplied(false)}
 className="flex-1 rounded-2xl px-4 py-2.5 text-sm font-semibold text-zinc-700 dark:border-zinc-700 dark:text-zinc-200"
 >
 Stay here
 </button>
 <Link
